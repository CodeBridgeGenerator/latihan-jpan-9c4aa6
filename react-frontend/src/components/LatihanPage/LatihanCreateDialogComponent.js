...
const LatihanCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            nomborRuj: _entity?.nomborRuj,tajuk: _entity?.tajuk,kategori: _entity?.kategori?._id,status: _entity?.status,
    ...
    return (
        <Dialog header="Create Latihan" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            {!_.isEmpty(error["nomborRuj"]) ? (
              <p className="m-0" key="error-nomborRuj">
                {error["nomborRuj"]}
              </p>
            ) : null}
          </small>
