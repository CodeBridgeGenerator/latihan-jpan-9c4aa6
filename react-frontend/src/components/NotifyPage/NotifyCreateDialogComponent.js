...
const NotifyCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            tajuk: _entity?.tajuk,venue: _entity?.venue,tarikh: _entity?.tarikh,pengajar: _entity?.pengajar,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="pengajar" className="w-full mb-3 p-inputtext-sm" value={_entity?.pengajar} onChange={(e) => setValByKey("pengajar", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["pengajar"]) ? (
              <p className="m-0" key="error-pengajar">
                {error["pengajar"]}
              </p>
            ) : null}
          </small>
